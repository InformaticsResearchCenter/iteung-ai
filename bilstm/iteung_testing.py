...
def setEncoderDecoder(VOCAB_SIZE):
    ...
    _, forward_h, forward_c, backward_h, backward_c = Bidirectional(LSTM(200, return_state=True, dropout=0.5, recurrent_dropout=0.5))(enc_embedding)
...
def make_inference_models(dec_lstm, dec_embedding, dec_dense, dec_inputs, enc_inputs, enc_states):
    dec_state_input_h = Input(shape=(200 * 2,))
    dec_state_input_c = Input(shape=(200 * 2,))
...
factory, stemmer, punct_re_escape, unknowns, file_name, path = setConfig("bilstm")
