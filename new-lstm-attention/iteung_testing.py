...
def setEncoderDecoder(VOCAB_SIZE):
    ...
    enc_outputs, state_h, state_c = LSTM(256, return_state=True, return_sequences=True, dropout=0.5,
                                         recurrent_dropout=0.5)(enc_embedding)

    enc_states = [state_h, state_c]

    dec_inputs = Input(shape=(None,))
    dec_embedding = Embedding(VOCAB_SIZE, 256, mask_zero=True)(dec_inputs)
    dec_lstm = LSTM(256, return_state=True, return_sequences=True, dropout=0.5, recurrent_dropout=0.5)
...
def setModel(enc_inputs, dec_inputs, output, dec_lstm, dec_embedding, dec_dense, enc_states, encoder_outputs,
             file_path):
...
dec_lstm, dec_embedding, dec_dense, dec_inputs, enc_inputs, enc_states, encoder_outputs, output = setEncoderDecoder(
    VOCAB_SIZE)
...
for i, v in data.iterrows():
    ...
    bot_a.append(botReply(val_question)[0])
...
bot_test_dataframe.to_csv('output_dir/test_result.csv', index=False)
